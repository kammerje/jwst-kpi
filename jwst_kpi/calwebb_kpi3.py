    ...
    def run(self, file):
        ...
        if not self.fix_bad_pixels.skip:
            suffix = self.fix_bad_pixels.step(
                file, suffix, self.output_dir, show_plots=self.show_plots
            )
...
class fix_bad_pixels:
    ...
    def step(self, file, suffix, output_dir, show_plots=False):
        ...
        print(
            ...
            % (np.sum(mask), np.sum(mask) / np.prod(mask.shape) * 100.0)
        )
...
class recenter_frames:
    ...
    def step(self, file, suffix, output_dir, show_plots=False):
        ...
        if self.method not in self.method_allowed:
            ...
            if data.ndim == 2:
                data_recentered, dx, dy = core.recenter(
                    data,
                    algo=self.method,
                    subpix=True,
                    between=False,
...
class window_frames:
    ...
    def step(self, file, suffix, output_dir, show_plots=False):
        ...
        sgmask = core.super_gauss(sy, sx, wrad, between_pix=False)
        ...
        if self.plot:
            ...
            if data.ndim == 2:
                p0 = ax[0].imshow(
                    np.log10(np.abs(data)), origin="lower", vmin=vmin, vmax=vmax
                )
            else:
                p0 = ax[0].imshow(
                    np.log10(np.abs(data[0])), origin="lower", vmin=vmin, vmax=vmax
                )
            plt.colorbar(p0, ax=ax[0])
            ax[0].set_title(
                "Full frame (log-scale)",
            ...
            if data.ndim == 2:
                ...
                p1 = ax[1].imshow(
                    np.log10(np.abs(data_windowed[0])),
                    origin="lower",
                    vmin=vmin,
                    vmax=vmax,
                )
    ...
    def step(
        ...
        if not recenter_frames_obj.skip:
            if recenter_frames_obj.method not in recenter_frames_obj.method_allowed:
                ...
                if data.ndim == 2:
                    dx, dy = KPO.extract_KPD_single_frame(
                        data,
                        PSCALE,
                        wave,
                        target=None,
        ...
        if not recenter_frames_obj.skip:
            ...
            if recenter_frames_obj.plot:
                ...
                ax[0, 0].set_title(
                    ...
                    pad=-20,
                    bbox=dict(
                        facecolor="white", edgecolor="lightgrey", boxstyle="round"
                    ),
                )
                if data.ndim == 2:
                    p01 = ax[0, 1].imshow(data_recentered, origin="lower")
                ...
                if data.ndim == 2:
                    ...
                    p11 = ax[1, 1].imshow(
                        data_recentered[0][yl:yu, xl:xu], origin="lower"
                    )
                plt.colorbar(p11, ax=ax[1, 1])
                ax[1, 1].axhline(sy // 2 - yl, color="red")
                ax[1, 1].axvline(sx // 2 - xl, color="red")
            ...
            if data.ndim == 2:
                xsh = pyfits.Column(
                    name="XSHIFT", format="D", array=np.array([dx])
                )  # pix
                ysh = pyfits.Column(
                    name="YSHIFT", format="D", array=np.array([dy])
                )  # pix
            else:
                xsh = pyfits.Column(
                    name="XSHIFT", format="D", array=np.array(dx)
        ...
        if not window_frames_obj.skip:
            ...
            if window_frames_obj.plot:
                ...
                if data.ndim == 2:
                    p0 = ax[0].imshow(
                        np.log10(np.abs(data_recentered)),
                        origin="lower",
                        vmin=vmin,
                        vmax=vmax,
                    )
                    ...
                    p0 = ax[0].imshow(
                        np.log10(np.abs(data_recentered[0])),
                        origin="lower",
                        vmin=vmin,
                        vmax=vmax,
                    )
        ...
        if self.plot:
            ...
            if data.ndim == 2:
                p01 = ax[0, 1].imshow(
                    kpcor, origin="lower", cmap="RdBu", vmin=-1.0, vmax=1.0
                )
            else:
                p01 = ax[0, 1].imshow(
        ...
        if "NIRCAM" in hdul[0].header["INSTRUME"]:
            hdul[0].header["GAIN"] = gain[
                hdul[0].header["INSTRUME"] + "_" + hdul[0].header["CHANNEL"]
            ]  # e-/ADU
        else:
        ...
        if data.ndim == 2:
            ...
            hdu_ang = pyfits.ImageHDU(
                np.array([hdul["SCI"].header["ROLL_REF"]] * data.shape[0])
            )  # deg
        hdu_ang.header["EXTNAME"] = "DETPA"
        hdul += [hdu_ang]
        if data.ndim == 2:
            hdu_vis = pyfits.ImageHDU(
                np.vstack(
                    (
                        np.real(KPO.CVIS[0])[np.newaxis, np.newaxis, :],
                        np.imag(KPO.CVIS[0])[np.newaxis, np.newaxis, :],
                    )
                )
            )
        else:
